...
echo
echo "Installing GnuPG for key verification..."
sudo apt-get install gpg -y
echo
echo "Downloading and installing the repository key..."
wget -qO- http://host.docker.internal:3000/keys/packhub.asc | gpg --dearmor > packhub.gpg
sudo install -D -o root -g root -m 644 packhub.gpg /etc/apt/keyrings/packhub.gpg
echo
echo "Adding the repository to your system..."
echo "deb [signed-by=/etc/apt/keyrings/packhub.gpg] http://host.docker.internal:3000/apt/github/{{owner}}/{{repo}} stable main" > /etc/apt/sources.list.d/{{repo}}.list
echo 
